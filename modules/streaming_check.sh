...
# WarpKit - 流媒体解锁检测模块 (无广告版本)
# 基于 RegionRestrictionCheck 项目，已移除所有广告内容
# 原项目: https://github.com/lmc999/RegionRestrictionCheck

################################################################################
# 以下是从 RegionRestrictionCheck 项目获取的完整检测代码，仅删除了广告相关内容
#################################################################################!/bin/bash
...
function MediaUnlockTest_ESPNPlus() {
    ...
    if [ -z "$tokenContent" ]; then
        echo -n -e "\r ESPN+:${Font_SkyBlue}${Font_Suffix}\t\t${Font_Red}Failed (Network Connection)${Font_Suffix}\n"
        return
    fi
    ...
    if [ -n "$isBlocked" ] || [ -n "$is403" ]; then
        echo -n -e "\r ESPN+:${Font_SkyBlue}${Font_Suffix}\t\t${Font_Red}No${Font_Suffix}\n"
        return
    fi
    ...
    if [ -z "$refreshToken" ]; then
        echo -n -e "\r ESPN+:${Font_SkyBlue}${Font_Suffix}\t\t${Font_Red}Failed (Error: PAGE ERROR)${Font_Suffix}\n"
        return
    fi
    ...
    if [ "$region" == 'US' ] && [ "$inSupportedLocation" == 'true' ]; then
        echo -n -e "\r ESPN+:${Font_SkyBlue}${Font_Suffix}\t\t${Font_Green}Yes${Font_Suffix}\n"
        return
    fi
